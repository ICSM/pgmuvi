...
class TestPeriodIndependentWavelengthCandidateComparisonReports(unittest.TestCase):
    def test_text_report_contains_contract_and_ranked_models(self):
        text = format_period_independent_wavelength_fit_candidate_comparison_report(
            _quality_score_report()
        )

        self.assertIn("Period-independent wavelength fit-candidate comparison", text)
    ...
    def test_lightcurve_methods_delegate_to_module_functions(self):
        ...
        via_method = lc.format_period_independent_wavelength_fit_candidate_comparison_report(
            report
        )
        via_function = format_period_independent_wavelength_fit_candidate_comparison_report(
            report
        )
        self.assertEqual(via_method, via_function)

        figures = lc.plot_period_independent_wavelength_fit_candidate_comparison(report)
        self.assertIn("score", figures)
        for fig in figures.values():
            fig.clf()
