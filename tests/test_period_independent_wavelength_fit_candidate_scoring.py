"""Tests for scoring period-independent wavelength fit-candidate runs."""
...
class TestPeriodIndependentWavelengthFitCandidateScoring(unittest.TestCase):
    def test_score_report_contract_is_advisory_and_nonselecting(self):
        report = score_period_independent_wavelength_fit_candidate_runs(_run_report())

        self.assertEqual(
            report["kind"], "period_independent_wavelength_fit_candidate_scores"
        )
    ...
    def test_candidate_results_alias_is_accepted(self):
        run_report = _run_report()
        run_report["candidate_results"] = run_report.pop("outcomes")

        report = score_period_independent_wavelength_fit_candidate_runs(run_report)

        self.assertEqual(report["n_scored"], 3)
        self.assertEqual(report["ranked_results"], report["scored_candidates"])
    ...
    def test_custom_weights_can_change_ranking(self):
        ...
        report = score_period_independent_wavelength_fit_candidate_runs(
    ...
    def test_method_matches_module_function(self):
        ...
        via_method = lc.score_period_independent_wavelength_fit_candidate_runs(_run_report())
        via_function = score_period_independent_wavelength_fit_candidate_runs(_run_report())
