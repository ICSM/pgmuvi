"""Tests for running period-independent wavelength fit candidates."""
...
def _candidate_report():
    return {
        ...
        "fit_candidates": [
            ...
            {
                "candidate_id": "rank2_2D_baseline",
                "rank": 2,
                "model": "2D",
...
class TestPeriodIndependentWavelengthFitCandidateRunner(unittest.TestCase):
    def test_runner_report_contract_is_advisory_and_nonselecting(self):
        ...
        report = lc.run_period_independent_wavelength_fit_candidates(
            candidate_report=_candidate_report(),
            fit_runner=runner,
        )

        self.assertEqual(
            report["kind"], "period_independent_wavelength_fit_candidate_results"
        )
    ...
    def test_module_function_matches_lightcurve_method(self):
        ...
        via_method = lc.run_period_independent_wavelength_fit_candidates(
            candidate_report=_candidate_report(), fit_runner=runner
        )
        via_function = run_period_independent_wavelength_fit_candidates(
            lc, candidate_report=_candidate_report(), fit_runner=runner
        )
        self.assertEqual(via_method, via_function)

    def test_candidate_results_alias_matches_outcomes(self):
        lc = _make_multiband_lightcurve()
        ...
        self.assertIn("candidate_results", report)
        self.assertEqual(report["candidate_results"], report["outcomes"])
    ...
    def test_fit_kwargs_are_passed_to_runner(self):
        ...
        lc.run_period_independent_wavelength_fit_candidates(
            candidate_report=_candidate_report(), fit_runner=runner
        )
    ...
    def test_candidate_limit_limits_attempts(self):
        lc = _make_multiband_lightcurve()
        seen = []
    ...
    def test_max_candidates_alias_limits_attempts(self):
        lc = _make_multiband_lightcurve()
        seen = []
    ...
    def test_rejects_non_candidate_report(self):
        lc = _make_multiband_lightcurve()

        with self.assertRaisesRegex(ValueError, "fit-candidate report"):
            lc.run_period_independent_wavelength_fit_candidates(
                candidate_report={"kind": "period_independent_wavelength_parameter_plan"}
            )

    def test_accepts_fit_candidate_report_alias(self):
        lc = _make_multiband_lightcurve()
        seen = []
    ...
    def test_can_build_candidate_report_when_not_supplied(self):
        lc = _make_multiband_lightcurve()
        seen = []
