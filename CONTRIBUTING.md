...
We use [pre-commit](https://pre-commit.com/) to automatically check code quality
before commits. To set up pre-commit hooks in your local environment:
...
After setup, pre-commit will automatically run on every commit. The hooks will:
...
If any hooks fail, the commit will be rejected. Fix the issues and try committing again.
