...
def _convert_time_to_days(xdata, time_units):
    ...
    xdata : torch.Tensor, numpy.ndarray, or array-like
        ...
        converted.  Non-tensor inputs are coerced to the package default floating
        dtype (``torch.float64``) automatically.
    ...
    if not isinstance(xdata, torch.Tensor):
        xdata = torch.as_tensor(xdata, dtype=DEFAULT_DTYPE)
...
class InputHelpers:
    ...
    def from_csv(
        ...
        **kwargs
        ...
        # ------------------------------------------------------------------
        # Helper: convert a structured-array column to the default floating tensor dtype.
        ...
        def _to_float_tensor(arr):
            return torch.as_tensor(
                np.array(arr, dtype=np.float64), dtype=DEFAULT_DTYPE
            )
        ...
        def _str_col_to_wave(arr):
            ...
            return (
                torch.as_tensor(indices, dtype=DEFAULT_DTYPE),
...
class Lightcurve(InputHelpers, gpytorch.Module):
    ...
    def _ensure_tensor(self, values):
        ...
        if not isinstance(values, torch.Tensor):
            ...
            values = torch.as_tensor(values, dtype=DEFAULT_DTYPE)
        return values
    ...
    def _move_module_to_data_dtype(self, module):
        """Cast a torch module to the lightcurve floating dtype when possible."""
        if isinstance(module, torch.nn.Module):
            module.to(dtype=self._floating_data_dtype())
        return module
        ...
        **kwargs,
        ...
        if likelihood == "learn":
            ...
            raise ValueError(
                ...
                "Pass a preconfigured likelihood object instead if custom "
        ...
        if _has_noise and likelihood is None:
            ...
            # With no per-point uncertainties, GaussianLikelihood already
            # learns a homoscedastic noise term. ``learn_additional_noise`` is
            # therefore satisfied by the standard likelihood rather than by
            # passing FixedNoise-only kwargs to GaussianLikelihood.
            self.likelihood = gpytorch.likelihoods.GaussianLikelihood()
    ...
    def set_model(
        ...
        **kwargs : dict, optional
        ...
        elif likelihood is None and _noise_setting_changed:
            self.set_likelihood(
                likelihood,
                variance=variance,
                learn_additional_noise=learn_additional_noise,
